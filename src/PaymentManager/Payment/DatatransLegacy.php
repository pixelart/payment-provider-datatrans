...
class DatatransLegacy extends AbstractPayment implements RecurringPaymentInterface
{
    public const TRANS_TYPE_DEBIT = '05';
    public const TRANS_TYPE_CREDIT = '06';

    public const AUTH_TYPE_AUTHORIZATION = 'NOA';
    public const AUTH_TYPE_FINAL_AUTHORIZATION = 'FOA'; // final authorization (MasterCard/Maestro)
    ...
    public function initPayment(PriceInterface $price, array $config)
    {
        ...
        $formAttributes['data-upp-start-target'] = $config['uppStartTarget'] ?: '_top';
        if ($config['useAlias']) {
     ...
     */
    public function handleResponse(array|StatusInterface $response): StatusInterface
    {
        ...
        $paymentState = null;
        if (\in_array($response['responseCode'], ['01', '02'], true)) {
            // success
            $paymentState = 'CAA' === $response['reqtype']
    ...
    public function executeDebit(?PriceInterface $price = null, ?string $reference = null): StatusInterface
    {
        ...
        if (\in_array($this->authorizedData['reqtype'], $this->getValidAuthorizationTypes(), true) && $this->authorizedData['uppTransactionId']) {
            // restore price object for payment status
        ...
        $message = null;
        $paymentState = null;
        if ('response' === $status && \in_array($response->responseCode, ['01', '02'], true)) {
